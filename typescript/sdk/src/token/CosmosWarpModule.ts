...
import { CosmosWarpRouteReader } from './CosmosWarpRouteReader.js';
...
export class CosmosWarpModule extends HyperlaneModule<
  ProtocolType.Cosmos,
  HypTokenRouterConfig,
  WarpRouteAddresses
> {
  protected logger = rootLogger.child({
    module: 'CosmosWarpModule',
  });
  reader: CosmosWarpRouteReader;
  ...
  constructor(
    ...
    super(args);
    this.reader = new CosmosWarpRouteReader(multiProvider, args.chain, signer);
  ...
  async deployOrUpdateIsm(
    ...
    deployedIsm: Address;
    updateTransactions: AnnotatedCosmJsTransaction[];
  }> {
   ...
   * @returns A new instance of the CosmosWarpModule.
   */
  static async create(params: {
    ...
    const warpModule = new CosmosWarpModule(
      multiProvider,
      {
        addresses: {
          deployedTokenRoute,
    ...
    if (config.remoteRouters && !isObjEmpty(config.remoteRouters)) {
      const enrollRemoteTxs = await warpModule.update(config); // @TODO Remove when CosmosWarpModule.create can be used
      const onlyTxIndex = 0;
