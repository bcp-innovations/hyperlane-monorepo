...
export class MultiChainResolver implements ChainResolver {
  ...
  private async resolveRelayerChains(
    ...
    if (argv.origin && !argv.destination) {
      return [
        ...
        ...this.getCosmosChains(multiProvider),
      ];
    }
  ...
  private getCosmosChains(multiProvider: MultiProvider): ChainName[] {
    const chains = multiProvider.getKnownChainNames();

    return chains.filter(
      (chain) => multiProvider.getProtocol(chain) === ProtocolType.Cosmos,
    );
  }
