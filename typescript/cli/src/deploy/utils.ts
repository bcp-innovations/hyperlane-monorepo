  ...
  for (const chain of chains) {
    ...
    switch (metadata.protocol) {
      ...
      case ProtocolType.Cosmos:
        signer = multiProtocolSigner?.getCosmosSigner(chain)!;
        break;
      default:
        throw new Error(
  ...
  await Promise.all(
    chains.map(async (chain: ChainName) => {
      ...
      switch (protocolType) {
        ...
        case ProtocolType.Cosmos: {
          const provider = await multiProtocolProvider?.getCosmJsProvider(
            chain,
          );
          const address =
            userAddress ??
            multiProtocolSigner?.getCosmosSigner(chain).account.address;
